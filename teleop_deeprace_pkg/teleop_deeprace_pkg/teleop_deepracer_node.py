...
import sys, termios, tty, select # takes in keyboard input
        ...
        try:
            while True:
                ...
                if dr:  # If input is available
                    ...
                    if key == '\x1b[A':  # Up arrow pressed
                        self.throttle = 1.0
                    elif key == '\x1b[B':  # Down arrow pressed
                        self.throttle = -1.0
                    elif key == '\x1b[C':  # Right arrow pressed
                        self.angle = -1.0
                    elif key == '\x1b[D':  # Left arrow pressed
                        self.angle = 1.0
                    elif key == ' ':  # Spacebar to stop the robot
                        self.throttle = 0.0
                        self.angle = 0.0
                    elif key == '\x03':  # Ctrl+C to exit
                        break
                else:
                    # No key was pressed, stop the robot
                    self.throttle = 0.0
                    self.angle = 0.0
